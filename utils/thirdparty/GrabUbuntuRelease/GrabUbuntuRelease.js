...
function parseToMD(
  ...
  if (version) {
    ...
    fs.appendFile(target, data, 'utf-8', function (err) {
      if (err) throw err
    })
  }
}
...
function parseTable(reqBody) {
  ...
  table[0].children.forEach((item, index) => {
    if (index !== 0) {
      ...
      let release_code_url =
        a_selector.eq(0).attr('href') === undefined
          ? ''
          : a_selector.eq(0).attr('href')
      ...
      let release_docs_url =
        a_selector.eq(1).attr('href') === undefined
          ? ''
          : a_selector.eq(1).attr('href')
      release_docs_url = parseURL(release_docs_url)
      // console.log(release_docs_url)
      const release_date = a_selector.eq(2).text()
      // console.log(release_date)
      let release_date_url =
        a_selector.eq(2).attr('href') === undefined
          ? ''
          : a_selector.eq(2).attr('href')
      ...
      let eos_date_url =
        a_selector.eq(3).attr('href') === undefined
          ? ''
          : a_selector.eq(3).attr('href')
...
async function prepare() {
  ...
  fs.readFile(template, function (err, data) {
    ...
    fs.writeFile(target, tmp, function (err) {
      if (err) throw err
    })
  })
}
